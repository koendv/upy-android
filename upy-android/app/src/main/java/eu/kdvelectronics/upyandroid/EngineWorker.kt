...
// exec()/reset() calls are serialized through it. interrupt() is the
// one operation that bypasses the queue, since it must be able to reach
// a script that is currently blocking the worker thread.
//
...
class EngineWorker(private val rootPath: String) {
    ...
    // nativeDeinit() touches the same interpreter and native state every
    // other queued call does. interrupt() is the exception, deliberately
